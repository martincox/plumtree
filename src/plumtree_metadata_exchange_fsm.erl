...
exchange(timeout, State=#state{peer=Peer}) ->
    RemoteFun = fun(Prefixes, {get_bucket, {Level, Bucket}}) ->
                   ...
                   (Prefixes, {key_hashes, Segment}) ->
                        plumtree_metadata_hashtree:key_hashes(Peer, Prefixes, Segment)
                end,
...
repair_sub_prefixes(Type, Peer, Prefix, It) ->
    case plumtree_metadata_manager:iterator_done(It) of
        true ->
        ...
        false ->
            ...
            repair_sub_prefixes(Type, Peer, Prefix,
                                plumtree_metadata_manager:iterate(It))
    end.
...
repair_keys(Peer, PrefixList, {_Type, KeyBin}) ->
    ...
    RemoteObj = plumtree_metadata_manager:get(Peer, PKey),
...
merge(undefined, PKey, RemoteObj) ->
    plumtree_metadata_manager:merge({PKey, undefined}, RemoteObj);
merge(Peer, PKey, LocalObj) ->
    plumtree_metadata_manager:merge(Peer, {PKey, undefined}, LocalObj).
...
remote_lock_request(Peer) ->
    Self = self(),
    as_event(fun() ->
                     Res = plumtree_metadata_hashtree:lock(Peer, Self),
                     {remote_lock, Res}
             end).
...
update_request(Node) ->
    as_event(fun() ->
                     ...
                     %% and tree is built
                     case plumtree_metadata_hashtree:update(Node) of
                         ok -> tree_updated;
